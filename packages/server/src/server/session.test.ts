...
test("directs provider subagent updates only to capable connection sources", () => {
  const messages: SessionOutboundMessage[] = [];
  ...
  const capableSource = {};
  const legacySource = {};
  session.updateClientCapabilities({ [CLIENT_CAPS.providerSubagents]: true }, capableSource);
  session.updateClientCapabilities({}, legacySource);

  if (!onAgentEvent) {
    throw new Error("Session did not subscribe to agent events");
  }
  onAgentEvent({
  ...
  expect(targetedMessages).toEqual([
    {
      ...
      message: {
        ...
        payload: {
          kind: "upsert",
          subagent: expect.objectContaining({ id: "provider-child", status: "running" }),
        },
      },
    },
  ]);
...
test("routes provider subagent snapshot responses to their request source", async () => {
  const messages: SessionOutboundMessage[] = [];
  const targetedMessages: Array<{ source: object; message: SessionOutboundMessage }> = [];
  ...
  expect(messages).toEqual([]);
  expect(targetedMessages.map((entry) => [entry.source, entry.message.type])).toEqual([
    [source, "agent.provider_subagents.list.response"],
    [source, "agent.provider_subagents.timeline.get.response"],
  ]);
});

test("does not use one source capability as a fallback for mixed sources", () => {
  const messages: SessionOutboundMessage[] = [];
  let onAgentEvent!: (event: AgentManagerEvent) => void;
  const session = createSessionForTest({
    messages,
    agentManager: {
      subscribe: vi.fn((listener: (event: AgentManagerEvent) => void) => {
